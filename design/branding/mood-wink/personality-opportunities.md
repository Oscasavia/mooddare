...
Reviewed against the current app on 2026-09-22. Rows marked implemented are now
in the app; the remaining rows are proposals. Behavior and recovery actions should
remain clear; the face is a supporting illustration.

| Existing place | Proposed expression | Use |
| --- | --- | --- |
| Moments: “The first moment could be yours” (implemented) | Encouraging smile, bright open eyes | Invite the first shared dare. |
| Moments filtered to an empty mood (implemented) | Thinking, raised eyebrow | Suggest another mood without implying an error. |
...
| Profile: “Your story starts here” (implemented) | Open-eyed smile | Give an empty moments grid an inviting introduction. |
| Comments: “Start the conversation” (implemented) | Talking face, small open mouth | Invite a first comment above the composer. |
| Followers/following: “No people here yet” (implemented) | Friendly open-eyed smile | Encourage connection; keep copy neutral on someone else's profile. A filtered list with no matches gets Thinking instead. |
...
The first empty-state group is installed. Mood searches are a possible next
extension of Thinking. Keep destructive confirmations, reporting,
