...
Welcome keeps the two sign-up/sign-in actions and uses an original offline mood-card illustration. Guest access is retired: the auth gate clears persisted anonymous sessions before showing Welcome, with a retry screen if sign-out fails. Registered accounts keep their sessions. Anonymous sign-in is disabled in the MoodDare Firebase project; the two legacy guest accounts were disabled and their refresh tokens revoked without deleting saved content. Previously issued ID tokens on older app versions can remain valid for up to one hour. The Google buttons use the unmodified multicolor logo from [Google’s official branding assets](https://developers.google.com/identity/branding-guidelines) on a white button. Design direction for the original welcome artwork: [playful dark onboarding](https://dribbble.com/shots/27037136-Cosmic-Social-App-Onboarding-UI-Dark-Mode-Exploration), retaining MoodDare’s existing type, layout and lavender palette.
